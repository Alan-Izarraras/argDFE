...
# For PresentSize
png(file.path(out_dir, "PresentSize.png"), width = 350, height = 600)
boxplot(Resultados$PresentSize, main = "Population size in present epoch", ylab = "", xaxt = "n", outline = FALSE)
dev.off()

# For PastSize
png(file.path(out_dir, sprintf("PastSize.png")), width = 350, height = 600)
boxplot(Resultados$PastSize, main = "Population size in past epoch", ylab = "", xaxt = "n", outline = FALSE)
dev.off()

# For TimeChange
png(file.path(out_dir, sprintf("TimeChange.png")), width = 350, height = 600)
...
png(file.path(out_dir, sprintf("PresentSize_density.png")),width = 541, height = 401)
options(scipen = 999)
density_est <- density(Resultados$PresentSize)
plot(density_est, main = "Present Size", xlab = "N", ylab = "Density", ylim = c(0, max(density_est$y) * 1))
...
png(file.path(out_dir, sprintf("PastSize_density.png")), width = 541, height = 401)
options(scipen = 999)
density_est <- density(Resultados$PastSize)
plot(density_est, main = "Past Size", xlab = "N", ylab = "Density", ylim = c(0, max(density_est$y) * 1.5))
...
# Time distributions
png(file.path(out_dir, sprintf("Time_density.png")), width = 541, height = 401)
options(scipen = 999)
density_est <- density(Resultados$Time)
plot(density_est, main = "Time change", xlab = "generations", ylab = "Density", ylim = c(0, max(density_est$y) * 1.5))
...
print("Posterior plots and demography file succesfully generated")
