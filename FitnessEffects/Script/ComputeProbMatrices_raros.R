###Lets add new frew distribution with rare aleles (1 to 10 copies on their own row)
...
#Take last 10 elemnts out of the full matrix. 
matriz_conteo_raras <- tail(matriz_conteo, n = 10)
#
...
write.csv(matriz_prob_invariables, paste(file_path, "set2_200x100_prob_matrix_Sel", sel, ".csv", sep=""), row.names = FALSE)
...
#matriz_raras <- matriz_conteo_rangos[-ultima,], + matriz_raras  
matriz_alelos_raros <- matriz_conteo_rangos[-nrow(matriz_conteo_rangos), ] 
matriz_alelos_raros <- rbind(matriz_alelos_raros, matriz_raras)
...
rownames(matriz_alelos_raros) <- c(1:nrow(matriz_alelos_raros))
matriz_prob_alelos_raros <- matriz_alelos_raros / max_effective_sites
matriz_alelos_raros <- rbind(matriz_alelos_raros, vector_1)
print("verificando alelos raros, si suma 1 todo bien.")
print(sum(matriz_prob_alelos_raros[,1]))
write.csv(matriz_prob_alelos_raros, paste(file_path, "set2_rarosx100_prob_matrix_Sel", sel, ".csv", sep=""), row.names = FALSE)
