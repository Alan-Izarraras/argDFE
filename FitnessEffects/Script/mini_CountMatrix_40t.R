...
for (a in 1:200) {
  ...
  min_value <- 0.000001
  max_value <- 1
  length <- 40 #change for different discrete times
...
#rangos_tiempo <- rangos_tiempo * 60000
  ...
  write.csv(matriz_conteo_invariables_2, paste(out_path, "200x40_count_",sel, formatC(a, width = 3, flag = "0"), "_sel", sel, ".csv", sep=""), row.names = FALSE)
  ...
  write.csv(matriz_conteo_rangos, paste(out_path, "20x40_count_",sel, formatC(a, width = 3, flag = "0"), "_sel", sel, ".csv", sep=""), row.names = FALSE)
