...
for (z in 1:50)  { #z handles repetition number
  ...
  for (a in (1:25))  { #Reads in every prob matrix (set2) #set2_200x100_prob_matrix_Sel1.csv
    ...
    prob_matrix <- prob_matrix[-nrow(prob_matrix), , drop = FALSE] #Erase last row becuase it contains the total number of sites and not a probability.
    for (b in (1:25)) {  #Reads in every count matrix (set1) 
      ...
      count_matrix <- count_matrix[-nrow(count_matrix), , drop = FALSE] #Erase last row becuase it contains the total number of sites but preserve matrix structure
      ...
      for (r in 1:nrow(count_matrix))  {  #Starts likelihood calc of this particular combination of matrices
        if (count_matrix[r, 1] != 0)  {
        likelihoods[i] <- count_matrix[r,1] * log(prob_matrix[r, 1]) #Necesito que este archivo no cambie
        i <- i +1
        }
      }
...
write.csv(result_mat, paste("ConstantSize_discrete/discrete_inference_", matrix_selection[task_id], "_SFS_cien.csv", sep=""))
