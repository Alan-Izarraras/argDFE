...
for (i in 1:27) {
  prob_matrix_list[[i]] <- read.csv(paste0("../../Data/trees/MatrixInputs/ConstantSize/set2/matrices/new_pseudocount/200x8_count_cien_kimDFE_Sel", i, ".csv"))
}

# Read in DFE_matrices (exactly as you had)                                                             
dfe_matrix <- read.csv(paste0("../../Data/trees/MatrixInputs/ConstantSize/set1/rep_matrices/boyko_lognormal/200x8_count_boykolognormDFE1_diezmil_rep", rep, ".csv"))
...
# Now fits a LOGNORMAL distribution (parameters: logmean / μ and logsd / σ)
neg_log_likelihood <- function(params) {
  logmean <- params[1]   # meanlog (μ)
  logsd   <- params[2]   # sdlog (σ)
  
  # Guard against invalid parameters
  if (logsd <= 0) return(1e20)
  ...
  for (bin in 1:n_bins) {
    if (bin == 1) {
      prob_mass <- plnorm(valor_medio[1], meanlog = logmean, sdlog = logsd)
    } else if (bin == n_bins) {
      prob_mass <- 1 - plnorm(valor_medio[bin - 1], meanlog = logmean, sdlog = logsd)
    } else {
      prob_mass <- plnorm(valor_medio[bin], meanlog = logmean, sdlog = logsd) - 
                   plnorm(valor_medio[bin - 1], meanlog = logmean, sdlog = logsd)
    }
...
# Bounds for lognormal (μ can be negative; σ must be positive)
# Generous range that easily covers your true values (logmean=5.02, logstddev=5.94)
lower_bounds <- c(-20, 0.01)
upper_bounds <- c(20,  30)
...
for (i in 1:n_starts) {
  ...
  # Uniform for logmean, log-uniform for logsd (much better than plain uniform!)
  start_logmean <- runif(1, min = lower_bounds[1], max = upper_bounds[1])
  start_logsd   <- 10^runif(1, log10(lower_bounds[2]), log10(upper_bounds[2]))
  
  start_params <- c(start_logmean, start_logsd)
  
  cat(sprintf("Start %d/%d:  logmean = %.4f   logsd = %.4f\n", 
              i, n_starts, start_logmean, start_logsd))
  ...
  opt_result <- optim(
    ...
    control = list(
      ...
      pgtol   = 1e-11,
      lmm     = 15,
      parscale = c(1, 1),   # tuned for lognormal parameters (both ~1-10 scale)
      ndeps   = c(1e-5, 1e-5)
    )
  )
...
cat("Best logmean     =", best_params[1], "\n")
cat("Best logsd       =", best_params[2], "\n")
...
write.table(data.frame(
  rep         = rep,
  logmean     = best_params[1],
  logsd       = best_params[2],
  loglik      = best_loglik,
  convergence = best_result$convergence,
  n_starts    = n_starts
),  
file = sprintf("boykolognorm_lognorm_tdiezmil_result_rep%02d.txt", rep),
