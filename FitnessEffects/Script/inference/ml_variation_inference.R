...
#input file paths for 200x100 matrices. 
set1_input <- "../../Data/trees/MatrixInputs/new_likelihood_experiment/set1/"
set2_input <- "../../Data/trees/MatrixInputs/new_likelihood_experiment/set2/"

task_id <- Sys.getenv("SLURM_ARRAY_TASK_ID")
task_id <- as.numeric(task_id)

matrix_selection <- c("200x100", "20x100", "6x100", "200x50", "20x50", "6x50", "200x8", "20x8", "6x8")
...
print(prob_matrix_name)

#This is fianl result that I plot from. 
total_sites <- 1000000

inference_matrix <- matrix(nrow=1, ncol=27)
classic_inference_matrix <- matrix(nrow=1, ncol=27)

#now for multiple repetitions I use for loop. 
#but before that I need randomizing script and whatnot. 
...
for (a in (1:27))  { #Reads in every prob matrix (set2) #set2_200x100_prob_matrix_Sel1.csv
  ...
  for (b in (1:27)) {  #Reads in every count matrix (set1)
    ...
    count_matrix <- read.csv(paste(count_matrix_name, b, ".csv", sep=""))
  ...
  inference_matrix <- rbind(inference_matrix, inference_vector) #did not work
...
write.csv(inference_matrix, paste(matrix_selection[task_id], "_discrete_inference.csv", sep="")) #did not work
