...
#200x100
results <- read.csv("discrete_inference_200x100_cien.csv")
results <- results[,-1]
...
#200x40
results <- read.csv("discrete_inference_200x40_cien.csv")
results <- results[,-1]
colnames(results) <- c(1:27)
...
p4 <- ggplot(matriz_maximos, aes(x = true_value, y = estimated_value)) +
    geom_count(alpha = 0.8, color = "#FDE725FF") +
    scale_size_area(max_size = 7) +
...
experiment_files <- tribble(
  ~experiment,           ~path,
  "100",  "discrete_inference_200x100_cien.csv",
  "40",          "discrete_inference_200x40_cien.csv",
  "8",          "discrete_inference_200x8_cien.csv",
  "1",      "discrete_inference_200x8_cien_SFS.csv"
)
...
p5 <- ggplot(log_rmse_df, aes(x = factor(true_label), y = log_raw_rmse, group = experiment)) +
  ...
  labs(
    ...
    title = expression("E) log"[10] ~ "(RMSE) across" ~ gamma ~ "values (" ~ theta ~ "= 100)")
  ) +
...
ggsave("Supplementary_figure_6_B.pdf", 
       plot = p5, 
