...
experiment_files <- tribble(
  ~experiment,           ~path,
  "t = 100",  "discrete_inference_200x100_mil.csv",
  "t = 40",          "discrete_inference_200x40_mil.csv",
  "t = 8",          "discrete_inference_200x8_mil.csv",
  "t = 1",      "discrete_inference_200x8_mil_SFS.csv"
)
...
p1 <- ggplot(log_rmse_df, aes(x = factor(true_label), y = log_raw_rmse, group = experiment)) +
  geom_line(aes(color = experiment, linetype = experiment), linewidth = 1.1, alpha = 0.7) +
  ...
  labs(
    ...
    title = expression("log"[10] ~ "(RMSE) for different discrete times" ~ theta ~ "= 1000")
  ) +
