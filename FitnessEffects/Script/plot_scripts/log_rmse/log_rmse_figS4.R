...
experiment_files <- tribble(
  ~experiment,           ~path,
  "t = 100",  "discrete_inference_200x100_diezmil.csv",
  "t = 40",          "discrete_inference_200x40_diezmil.csv",
  "t = 8",          "discrete_inference_200x8_diezmil.csv",
  "t = 1",      "discrete_inference_200x8_diezmil_SFS.csv"
)
...
p1 <- ggplot(log_rmse_df, aes(x = factor(true_label), y = log_raw_rmse, group = experiment)) +
  ...
  labs(
    ...
    title = expression("log"[10] ~ "(RMSE) for different discrete times")
  ) +
