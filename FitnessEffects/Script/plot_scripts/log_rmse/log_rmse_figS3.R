...
experiment_files <- tribble(
  ~experiment,           ~path,
  "35_F",  "discrete_inference_6x100_diezmil.csv",
  "10_F",          "discrete_inference_20x100_diezmil.csv",
  "1_F",          "discrete_inference_200x100_diezmil.csv",
)
...
p1 <- ggplot(log_rmse_df, aes(x = factor(true_label), y = log_raw_rmse, group = experiment)) +
  ...
  labs(
    ...
    title = expression("log"[10] ~ "(RMSE) across F values")
  ) +
