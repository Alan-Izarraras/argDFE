...
#De las matrices construidas con 10k theta
#3 paneles.
...
results <- read.csv("discrete_inference_6x8_diezmil.csv")
results <- results[,-1]
...
p1 <- ggplot(matriz_maximos, aes(x = true_value, y = estimated_value)) +
    ...
    labs(size = "Simulation \n replicates", x = expression("Real value of" ~ gamma), y = expression("Estimated value of" ~ gamma), title = expression("A)" ~ F^35 * ", K = 8 different time points")) +
...
results <- read.csv("discrete_inference_20x8_diezmil.csv")
results <- results[,-1]
colnames(results) <- c(1:27)
...
p2 <- ggplot(matriz_maximos, aes(x = true_value, y = estimated_value)) +
    geom_count(alpha = 0.5, color = "#21908C") +
    scale_size_area(max_size = 6) +
    labs(size = "Simulation \n replicates", x = expression("Real value of" ~ gamma), y = expression("Estimated value of" ~ gamma), title = expression("B)" ~ F^10 * ", K = 8 different time points")) +
    # Force identical breaks **and** limits on both axes
    scale_x_continuous(breaks = breaks, 
                       labels = c(0,"","","",0.01,"","","",0.1,"","","",1,"","","",10,"","","",100,"","","",1000,"", ""),
                       limits = c(min(breaks), max(breaks))) +
...
p3 <- ggplot(matriz_maximos, aes(x = true_value, y = estimated_value)) +
    geom_count(alpha = 0.8, color = "#FDE725") +
    scale_size_area(max_size = 6) +
    labs(size = "Simulation \n replicates", x = expression("Real value of" ~ gamma), y = expression("Estimated value of" ~ gamma), title = expression("C)" ~ F^1 * ", K = 8 different time points")) +
    # Force identical breaks **and** limits on both axes
    scale_x_continuous(breaks = breaks, 
                       labels = c(0,"","","",0.01,"","","",0.1,"","","",1,"","","",10,"","","",100,"","","",1000,"", ""),
                       limits = c(min(breaks), max(breaks))) +
...
experiment_files <- tribble(
  ~experiment,           ~path,
  "F^35",  "discrete_inference_6x100_diezmil.csv",
  "F^10",          "discrete_inference_20x40_diezmil.csv",
  "F^1",          "discrete_inference_200x8_diezmil.csv",
)
...
p5 <- ggplot(log_rmse_df, aes(x = factor(true_label), y = log_raw_rmse, group = experiment)) +
  ...
  scale_color_viridis_d(name = "groupings") +  
  ...
  labs(
    ...
    title = expression("E) log"[10] ~ "(RMSE) across" ~ gamma ~ "values")
  ) +
...
# Save
ggsave("Supplementary_figure_2.pdf", 
       plot = combined, 
       width = 19, height = 6,   # Wider for 2x2
       dpi = 300)

ggsave("Supplementary_figure_2_B.pdf", 
       plot = p5, 
