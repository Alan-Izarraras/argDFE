...
#This is supplementary graph 4 not 3 ! 
...
results <- read.csv("../inference/ConstantSize/discrete_inference_200x100_diezmil.csv")
results <- results[,-1]
...
p1 <- ggplot(matriz_maximos, aes(x = true_value, y = estimated_value)) +
    geom_count(alpha = 0.6, color = "#440154FF") +
    ...
    theme(
        ...
        plot.margin = margin(1,1,1,1)  # top, right, bottom, left (in mm)
      )

###Plot number 2 (K^10 aka 20x8)
results <- read.csv("../inference/ConstantSize/discrete_inference_200x40_diezmil.csv")
results <- results[,-1]
colnames(results) <- c(1:27)
...
p2 <- ggplot(matriz_maximos, aes(x = true_value, y = estimated_value)) +
    geom_count(alpha = 0.6, color = "#414487FF") +
...
experiment_files <- tribble(
  ~experiment,           ~path,
  "100",  "discrete_inference_200x100_diezmil.csv",
  "40",          "discrete_inference_200x40_diezmil.csv",
  "8",          "discrete_inference_200x8_diezmil.csv",
  "1",      "discrete_inference_200x8_diezmil_SFS.csv"
)
...
ggsave("Supplementary_figure_3_B.pdf", 
    plot = p5, 
    width = 12, height = 6,   # Wider for 2x2
    dpi = 300)

# Save
ggsave("Supplementary_figure_3_A.pdf", 
       plot = combined, 
