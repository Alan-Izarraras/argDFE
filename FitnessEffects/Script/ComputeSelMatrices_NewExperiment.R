...
#this produces set1 and set2 matrices with different row numbers.
...
out_path <- "../Data/trees/MatrixInputs/new_likelihood_experiment/second_run/set2"
...
#heres the error and its saying the last line does not ahve 2 elements
#but the last line is empty. 
#necesito revisarlo cuidadosamente. 
...
#Since this is all simulatd data I can skip adjustment..? Eitherway doest interfere.
...
rownames(matriz_prob_invariables) <- c(1:200)
#write.csv(matriz_conteo_invariables, paste(file_path, "set2_200x8_prob_matrix_Sel", sel, ".csv", sep=""), row.names = FALSE)
write.csv(matriz_prob_invariables, paste(file_path, "set2_200x8_prob_matrix_Sel", sel, ".csv", sep=""), row.names = FALSE)


#n rows. How many rows are grouped together 
#50 times full, 20 and ... mixed? 
rows_per_part <- 10
...
matriz_prob_invariables_rangos <- matriz_conteo_rangos / max_effective_sites
...
rownames(matriz_conteo_invariables_rangos) <- c(1:nrow(matriz_conteo_invariables_rangos))
matriz_prob_invariables_rangos <- rbind(matriz_prob_invariables_rangos, vector_1)
rownames(matriz_prob_invariables_rangos) <- c(1:nrow(matriz_prob_invariables_rangos))
#write.csv(matriz_conteo_invariables_rangos, paste(file_path, "observed_20x8_count_matrix_Sel", sel, ".csv", sep=""), row.names = FALSE)
write.csv(matriz_prob_invariables_rangos, paste(file_path, "set2_20x8_prob_matrix_Sel", sel, ".csv", sep=""), row.names = FALSE)
...
rownames(matriz_prob_rangos) <- c(1:nrow(matriz_prob_rangos))
#write.csv(matriz_conteo_rangos, paste(file_path, "observed_6x8_count_matrix_Sel", sel, ".csv", sep=""), row.names = FALSE)
write.csv(matriz_prob_rangos, paste(file_path, "set_6x8_prob_matrix_Sel", sel, ".csv", sep=""), row.names = FALSE)
